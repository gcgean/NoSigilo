-- NoSigilo · Conversao de HOMENS cadastrados nos ultimos 7 dias.
-- Uso: docker exec -i nosigilo-postgres psql -U nosigilo -d nosigilo < analise_homens_7d.sql
...
SELECT count(*) AS nao_assinantes,
  ...
  count(*) FILTER (WHERE last_seen::date > created_ts::date) AS voltaram_outro_dia,
  count(*) FILTER (WHERE last_seen > now() - interval '48 hours') AS ativos_ult_48h
...
\echo '=== 5_POR_DIA ==='
SELECT created_ts::date AS dia, count(*) AS homens, count(*) FILTER (WHERE paid) AS assinaram
