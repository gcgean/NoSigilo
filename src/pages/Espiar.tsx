...
const ROTULO_INTERESSE: Record<string, string> = {
  'Casal (Ele/Ela)': 'casais',
  Mulher: 'mulheres',
  Homem: 'homens',
  outros: 'perfis trans, CD e travestis',
};
...
export default function Espiar() {
  ...
  useEffect(() => {
    if (!uf) return;
    void espiarService.perfis(uf, interesse).then((r) => setPerfis(r.perfis)).catch(() => setPerfis([]));
    void espiarService.regiao(uf, interesse).then((r) => setTotal(r.cadastrados)).catch(() => setTotal(0));
  }, [uf, interesse]);
      ...
      <main className="mx-auto max-w-4xl space-y-4 px-4 py-5">
        {total > 0 && (
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-center">
            <p className="text-3xl font-bold text-brand-pink">{total.toLocaleString('pt-BR')}</p>
            <p className="text-sm">
              {ROTULO_INTERESSE[interesse] ?? 'perfis'} cadastrados {uf ? `em ${uf}` : 'na sua região'}
            </p>
          </div>
        )}
